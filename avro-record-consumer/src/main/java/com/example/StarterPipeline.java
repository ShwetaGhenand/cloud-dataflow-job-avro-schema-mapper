...
import com.example.subprocess.AvroDeserializer;
...
public class StarterPipeline {
  ...
  public static void runDataPipelineFn(MyOptions options) {
    ...
    try {
      ...
      PCollection<KafkaRecord<String, byte[]>> kafkaRecord = pipeLine
          .apply(KafkaIO.<String, byte[]>read().withBootstrapServers(options.getKafkaURL())
              .withTopic(options.getTopicName()).withKeyDeserializer(StringDeserializer.class)
              .withValueDeserializer(ByteArrayDeserializer.class));
      kafkaRecord.apply(ParDo.of(new AvroDeserializer()));
      pipeLine.run();
    } catch (Exception e) {
